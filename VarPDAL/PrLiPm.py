...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 13 20:32:18 2023
...
def PDHG_PrLiPm(model,oracle,options,tol,maxiter,check):
    ...
    gradK_k = gradK(model, options,x_k);
    grad_kp1 = grad_k
    gradK_kp1 = gradK_k
    ...
    ########################## line search ################################
        ...
        for i in range(1,10+1):
            ...
            if iter<(m+1):
                ...
                norm_sq = (diff ).dot(B0*(diff )) + vector1.dot(vector1) - vector2.dot(vector2)
        ...
        gradK_kp1 = gradK(model, options,x_kp1);
        if breakvalue == 1:
...
def gradK(model, options,x_kp1):
    K = model['K'];
    return K.T.dot(K.dot(x_kp1))+x_kp1
