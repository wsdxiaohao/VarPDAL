...
def PDHG_VaLiPm_acc(model,oracle,options,tol,maxiter,check):
    ...
    ########################## line search ################################
        ...
        for i in range(1,linesearch_iter+1):
            ...
            if iter<(m+1):
                ...
                norm_sq2 = norm_sq1 +vector1.dot(vector1) - vector2.dot(vector2)
        ...
        if iter >= (m):
             ...
             #saling:
             #U2diag = np.diag(U2.T.dot(U2))
             #Eigmax
             
             r = 1#m*0.9/np.sum(U2**2)
             print('*********** scaling r = *******)')
             print(r)
                 
             Metric['U1'] = 1 * Metric['U1']       
             Metric['U2'] = 1 * Metric['U2'] 
